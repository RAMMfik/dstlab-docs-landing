...
export function AdminBillingRefundButton({
  ...
  async function handleRefund() {
    const confirmed = window.confirm(
      "Выполнить возврат средств по этому платежу?"
    );
    ...
    try {
      ...
      const response = await fetch(`/api/admin/billing/refund/${paymentId}`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !result.success) {
        window.alert(result.message ?? "Не удалось выполнить возврат.");
        return;
      }

      window.alert(
        `Возврат выполнен. Новый статус платежа: ${result.status ?? "UNKNOWN"}`
      );
      window.location.reload();
    } catch {
      window.alert("Ошибка возврата. Попробуйте позже.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <button
      ...
      title="Возврат средств по платежу"
    >
      {isLoading ? "Возврат..." : "Возврат"}
    </button>
  );
}